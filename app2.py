    ...
    if input_list:
        ...
        if st.button("🚀 開始完整診斷", use_container_width=True):
            ...
            if results_for_excel:
                ...
                wb.save(output)
                st.download_button(label="📥 下載 E1.4.3 完整報表", data=output.getvalue(), file_name=f"Fish_E1.4.3_{datetime.date.today()}.xlsx", use_container_width=True)