    ...
    if input_list:
        ...
        for idx, q in enumerate(input_list):
            with cols[idx % 3]:
                ...
                if search_res:
                    # 將結果排序：代碼結尾包含 .TW 或 .TWO 的權重設為 0，其餘為 1
        ...
        if st.button("🚀 開始完整診斷", use_container_width=True):
            ...
            # --- Excel 專業匯出 (Row 1-4 橘色 / Row 10+ 藍色) ---
            if results_for_excel:
                ...
                for r_idx, row_raw in enumerate(results_for_excel, header_row + 1):
                    ...
                    if "USD" not in sid:
                        r_df = get_finmind_data("TaiwanStockMonthRevenue", sid_clean, "2024-11-01")
                        if not r_df.empty:
                    ...
                    row_vals = [r_idx-header_row, sid, cp, f"{((cp-bp)/bp)*100:+.2f}%", bull_items if bull_items else "無", bear_items if bear_items else "無", rev_summary, eps_summary, row_raw[10]]
